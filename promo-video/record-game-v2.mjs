...
const OUT = 'D:/life-pawnshop/promo-video/footage/gameplay-v2';
...
// 召唤陪玩
const bots = spawn('node', ['bots-join.cjs', code], { cwd: 'D:/life-pawnshop/promo-video', stdio: 'inherit' });
// 等两位进房
...
while (!done && Date.now() < deadline) {
  ...
  try {
    ...
    if (!picked && await page.locator('h3:has-text("选出 7 张")').count()) {
      if (!globalThis.pickStarted) { mark('pick-start'); globalThis.pickStarted = true; }
      const st = await page.evaluate(() => ({
        sel: document.querySelectorAll('.panel .qcard.sel:not(.custom)').length,
        customs: document.querySelectorAll('.panel .qcard.sel.custom').length,
      }));
      if (st.sel < 6) {
        const un = page.locator('.panel .qcard:not(.mini):not(.sel):not(.custom)').first();
        if (await un.count()) { await tapLoc(un); await page.waitForTimeout(380); }
        continue;
      }
      if (st.customs === 0) {
        ...
        if (await cust.count()) {
          await tapLoc(cust);
          await cust.pressSequentially('从一而终的梦想', { delay: 200 });
      ...
      if (await ok.count()) {
        ...
        if (!(await page.locator('h3:has-text("选出 7 张")').count())) { picked = true; mark('pick-done'); }
      }
      continue;
    }
    // 暗拍
    ...
    if (await pawnBtn.count()) {
      ...
      if (myDramas === 1) {
        mark('my-drama-pawn');
        const dream = page.locator('.pawnbox .qcard', { hasText: '梦想' });
        const cs = page.locator('.pawnbox .qcard');
        if (await dream.count()) { await tapLoc(dream.first()); } else { await tapLoc(cs.nth(0)); }
        await page.waitForTimeout(500);
        const other = page.locator('.pawnbox .qcard:not(.sel)');
        await tapLoc((await other.count()) ? other.first() : cs.nth(1)); await page.waitForTimeout(600);
        const pb = page.locator('button:has-text("典当这 2 张"):not([disabled])');
        if (await pb.count()) await tapLoc(pb);
        await page.waitForTimeout(2200);
        try { mark('verdict:' + (await page.locator('#ownerline').innerText()).trim()); } catch {}
      } else {
